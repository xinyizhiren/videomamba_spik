        ...
        if self.mode == 'train':
            ...
            if args.num_sample > 1:
                ...
                for _ in range(args.num_sample):
                    ...
                    # 2. 生成单帧重复的伪视频序列
                    # 随机选择一帧
                    random_idx_1 = np.random.randint(0, new_frames_view1.shape[1])  # 在 T 维度随机选索引
                    selected_frame_1 = new_frames_view1[:, random_idx_1, :, :]  # 形状 (C, H, W)
                    # print(f'selected_frame shape（CHW）: {selected_frame.shape}')
                    # 沿时间轴重复，生成 T 帧
                    spat_frames_view1 = selected_frame_1.unsqueeze(1).expand(-1, new_frames_view1.shape[1]//2 , -1, -1)  # 形状 (C, T, H, W)
                    # 存储结果
                    spat_list_clip1.append(spat_frames_view1)

                    random_idx_2 = np.random.randint(0, new_frames_view2.shape[1])  # 在 T 维度随机选索引
                    selected_frame_2 = new_frames_view2[:, random_idx_2, :, :]  # 形状 (C, H, W)
                    # 沿时间轴重复，生成 T 帧
                    spat_frames_view2 = selected_frame_2.unsqueeze(1).expand(-1, new_frames_view2.shape[1] //2, -1, -1)  # 形状 (C, T, H, W)
                    # 存储结果
                    spat_list_clip2.append(spat_frames_view2)
                ...
                # print(
                #     "\n[调试] 单个样本返回形状：",
                #     f"- frame_list_view1 长度: {len(frame_list_view1)} (增强样本数)",
                #     f"- frame_list_view1[0]每个增强样本帧形状: {frame_list_view1[0].shape}  (C, T, H, W)",
                #     f"- spat_list_clip1 长度: {len(spat_list_clip1)} (应与增强样本数一致)",
                #     f"- 每个spat_frames_clip1[0]形状: {spat_list_clip1[0].shape} (应与原始buffer相同)",
                #     f"- spat_list_clip2 长度: {len(spat_list_clip2)} (应与增强样本数一致)",
                #     f"- 每个spat_frames_clip2[0]形状: {spat_list_clip2[0].shape} (应与原始buffer相同)",
                #     f"- 标签数: {len(label_list)}, 索引数: {len(index_list)}",
                #     sep="\n"
                # )

                return frame_list_view1, frame_list_view2, label_list, spat_list_clip1,spat_list_clip2,{}
            else:

                buffer_view1 = self._aug_frame(buffer_view1, args)
                buffer_view2 = self._aug_frame(buffer_view2, args)

                # 在 else 分支（num_sample <=1）返回前添加
                print(f"buffer_view1形状: {buffer_view1.shape} (期望 [3,16,224,224])")
                print(f"spat_frames_view1形状: {spat_frames_view1.shape} (期望 [3,16,224,224])")

                # 随机选择一帧
                random_idx_1 = np.random.randint(0, buffer_view1.shape[1])  # 在 T 维度随机选索引
                selected_frame_1 = buffer_view1[:, random_idx_1, :, :]  # 形状 (C, H, W)
                # print(f'selected_frame shape（CHW）: {selected_frame.shape}')
                # 沿时间轴重复，生成 T 帧
                spat_frames_view1 = selected_frame_1.unsqueeze(1).expand(-1, buffer_view1.shape[1], -1,
                                                                         -1)  # 形状 (C, T, H, W)


                random_idx_2 = np.random.randint(0, buffer_view2.shape[1])  # 在 T 维度随机选索引
                selected_frame_2 = buffer_view2[:, random_idx_2, :, :]  # 形状 (C, H, W)
                # 沿时间轴重复，生成 T 帧
                spat_frames_view2 = selected_frame_2.unsqueeze(1).expand(-1, buffer_view2.shape[1] , -1,
                                                                         -1)  # 形状 (C, T, H, W)


            return buffer_view1, buffer_view2, self.label_array[index], spat_frames_view1, spat_frames_view2, {}
