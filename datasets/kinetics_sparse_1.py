...
class VideoClsDataset_sparse(Dataset):
    ...
    def __init__(self, anno_path_view1,anno_path_view2, prefix='', split=' ', mode='train', clip_len=8,
        ...
        self.label_array = list(cleaned_view1.values[:, 1])  # 假设两个视角的标签相同
        ...
        if (mode == 'train'):
            ...
            self.test_seg = []
            self.test_dataset_view1 = []
            self.test_dataset_view2 = []
            ...
            for ck in range(self.test_num_segment):
                for cp in range(self.test_num_crop):
                    for idx in range(len(self.label_array)):
                        ...
                        self.test_dataset_view1.append(self.dataset_samples_view1[idx])
                        self.test_seg.append((ck, cp))


                        self.test_dataset_view2.append(self.dataset_samples_view2[idx])
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            if args.num_sample > 1:
                ...
                index_list = []
                for _ in range(args.num_sample):
                    new_frames_view1 = self._aug_frame(buffer_view1, args)
                    ...
                    label_list.append(label)
                    index_list.append(index)
                return frame_list_view1, frame_list_view2, label_list, index_list, {}
            else:
            ...
            sample_view1 = self.test_dataset_view1[index]  # 视角1
            sample_view2 = self.test_dataset_view2[index]  # 视角2
            chunk_nb, split_nb = self.test_seg[index]
            # buffer = self.loadvideo_decord(sample, chunk_nb=chunk_nb)
            # 加载视角1和视角2的视频数据
            buffer_view1 = self.loadvideo_decord(sample_view1, chunk_nb=chunk_nb)
            buffer_view2 = self.loadvideo_decord(sample_view2, chunk_nb=chunk_nb)

            # while len(buffer) == 0:
            while len(buffer_view1) == 0 or len(buffer_view2) == 0:
                warnings.warn("video {}, temporal {}, spatial {} not found during testing".format(\
                    str(self.test_dataset_view1[index]), chunk_nb, split_nb))
                index = np.random.randint(self.__len__())
                # sample = self.test_dataset[index]
                sample_view1 = self.test_dataset_view1[index]
                sample_view2 = self.test_dataset_view2[index]
                chunk_nb, split_nb = self.test_seg[index]
                # buffer = self.loadvideo_decord(sample, chunk_nb=chunk_nb)
                buffer_view1 = self.loadvideo_decord(sample_view1, chunk_nb=chunk_nb)
                buffer_view2 = self.loadvideo_decord(sample_view2, chunk_nb=chunk_nb)
            ...
            buffer_view1 = self.data_resize(buffer_view1)
            buffer_view2 = self.data_resize(buffer_view2)
            ...
            # # 如果是列表类型，将其转换为数组
            if isinstance(buffer_view1, list):
                buffer_view1 = np.stack(buffer_view1, 0)

                buffer_view2 = np.stack(buffer_view2, 0)

            # 合并视角1和视角2的数据（你可以使用不同的方式，比如 `np.stack` 或 `torch.cat`）假设我们有以下数据：
            #
            # buffer_view1：形状 (8, 3, 224, 224)
            # buffer_view2：形状 (8, 3, 224, 224)
            # 合并操作后，buffer 的形状将变为 (16, 3, 224, 224)，即包含了16个样本的数据。
            # buffer = np.concatenate([buffer_view1, buffer_view2], axis=0)  # 视角数据合并

            # print(f'buffer_view1 shape: {buffer_view1.shape}')
            # print(f'buffer_view2 shape: {buffer_view2.shape}')
            if self.test_num_crop == 1:
                spatial_step = 1.0 * (max(buffer_view1.shape[1], buffer_view1.shape[2]) - self.short_side_size) / 2
                spatial_start = int(spatial_step)

            else:
                spatial_step = 1.0 * (max(buffer_view1.shape[1], buffer_view1.shape[2]) - self.short_side_size) \
                                    / (self.test_num_crop - 1)
                spatial_start = int(split_nb * spatial_step)
            if buffer_view1.shape[1] >= buffer_view1.shape[2]:
                buffer_view1 = buffer_view1[:, spatial_start:spatial_start + self.short_side_size, :, :]
                buffer_view2 = buffer_view2[:, spatial_start:spatial_start + self.short_side_size, :, :]
            else:
                buffer_view1 = buffer_view1[:, :, spatial_start:spatial_start + self.short_side_size, :]
                buffer_view2 = buffer_view2[:, :, spatial_start:spatial_start + self.short_side_size, :]
            ...
            # print(f'buffer_view1 shape: {buffer_view1.shape}')
            # print(f'buffer_view2 shape: {buffer_view2.shape}')
            return buffer_view1,buffer_view2, self.test_label_array[index], sample_view1.split("/")[-1].split(".")[0], \
                   chunk_nb, split_nb
        else:
    ...
    def __len__(self):
        if self.mode != 'test':
            print(f"Dataset length (mode: {self.mode}): {len(self.dataset_samples_view1)}")
            # return len(self.dataset_samples_view1 + self.dataset_samples_view2)
            return len(self.dataset_samples_view1)
        else:
            print(f"view1 Dataset length (mode: {self.mode}): {len(self.test_dataset_view1) }")
            print(f"view2 Dataset length (mode: {self.mode}): {len(self.test_dataset_view2)}")

            return len(self.test_dataset_view1)
