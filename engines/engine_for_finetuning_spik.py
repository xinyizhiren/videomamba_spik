...
def train_class_batch(model, samples_view1, samples_view2, target, criterion, cur_epoch):
    ...
    if samples_view2 is None:
        fused_logits, _ = model(samples_view1)
        loss_fusion = criterion(fused_logits, target)
        return loss_fusion, fused_logits

    fused_logits, loss_hsic, (alpha1, alpha2),_ = model(samples_view1, samples_view2)

    loss_fusion = criterion(fused_logits, target)
    
    
    loss_single = ce_loss(target, alpha1 + 1, 12, cur_epoch, 20) + \
               ce_loss(target, alpha2 + 1, 12, cur_epoch, 20)

    total_loss = (
        loss_fusion + 0.7*loss_single + 5*loss_hsic
    )

    # -------------------------- 5. 整理损失字典（用于日志记录） --------------------------
    loss_dict = {
        "total_loss": total_loss.item(),
        "fusion_loss": loss_fusion.item(),
        "single_loss": loss_single.item(),
        "hsic_loss": loss_hsic.item() if model.training else 0.0,
        "hsic_sigma": model.hsic_criterion.sigma_x if model.training else 0.0
    }
    print("loss_dict:", loss_dict)

    return total_loss, fused_logits
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples_view1, samples_view2, targets, _,_) in enumerate(
        ...
        if dist.is_available() and dist.is_initialized():
            ...
            dist.all_gather(loss_list, loss)  # 分布式环境下执行all_gather
        else:
            world_size = 1  # 单机训练时设为1
            loss_list = [torch.zeros_like(loss) for _ in range(world_size)]
            loss_list[0] = loss  # 单机环境下直接赋值，不调用dist.all_gather！

        # 注意：loss_list是tensor列表，需用torch.stack转换为tensor，而非直接torch.tensor()
        ...
        if loss_list_isnan or loss_list_isinf:
            ...
            print("Loss is {}, stopping training".format(loss.item()))  # 用loss.item()获取数值
            sys.exit(1)
...
def final_test(data_loader, model, device, file, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    target_layer = base_model.temporal_branch.patch_embed.proj_linear  # 选择 patch_embed 的卷积层
