...
def train_class_batch(model, samples_view1, samples_view2, target, criterion, cur_epoch):
    ...
    # lossnds1 = ce_loss(target, alpha1 + 1, 8, cur_epoch, 10) + \
    #            ce_loss(target, alpha2 + 1, 8, cur_epoch, 10)
    # # c是类别
    # print(f"crossloss: {loss}")
    # print(f"0*lossnds1: {0 * lossnds1}")
    # total_loss = 0 * lossnds1 + loss
    ...
    print(f"10edlloss value: {10*loss1}")
    total_loss = 10*loss1 + loss
...
def is_overlapping(box1, box2, min_distance=0):
    """检查两个边界框是否重叠
    box1, box2: (x1, y1, x2, y2) 格式的边界框
    min_distance: 两个框之间的最小距离
    """
    x1_1, y1_1, x2_1, y2_1 = box1
    x1_2, y1_2, x2_2, y2_2 = box2
    
    # 检查是否重叠
    return not (x2_1 + min_distance <= x1_2 or x2_2 + min_distance <= x1_1 or 
                y2_1 + min_distance <= y1_2 or y2_2 + min_distance <= y1_1)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # 新增：定义缩略图大小和应用概率
    thumbnail_size = 56  # 可根据需要调整缩略图大小
    participation_rate = 0.8  # 80% 的概率应用缩略图嵌入
    print(" thumbnail_size", thumbnail_size)
    print("participation_rate",participation_rate)
    
    # 新增：控制缩略图数量和嵌入方向的参数
    num_thumbnails = 3  # 控制嵌入缩略图的数量，可以设置为任意正整数
    bidirectional_embedding = True  # True为双向嵌入（互嵌），False为单向嵌入（视角2嵌入视角1）
    print("num_thumbnails", num_thumbnails)
    print("bidirectional_embedding", bidirectional_embedding)
    ...
    for data_iter_step, (samples_view1, samples_view2, targets, _, _,_) in enumerate(
        ...
        if random.random() < participation_rate:
            ...
            # 新增：定义叠加透明度
            overlay_alpha = 0.6  # 缩略图权重，可调整为0.5或其他值
            original_alpha = 1.0 - overlay_alpha  # 原始图像权重
            print("overlay_alpha:",overlay_alpha)

            # 定义连续帧的长度（例如取 3 帧）
            seq_length = min(5, num_frames)  # 确保不超过总帧数
            ...
            for b in range(batch_size):
                ...
                # 生成指定数量的缩略图和边界框
                for thumb_idx in range(num_thumbnails):
                    # 随机选择起始帧索引
                    ...
                    for attempt in range(max_attempts):
                        ...
                        for existing_box in boxes:
                            if is_overlapping(box, existing_box, min_distance=2):
                                overlap = True
                                break
                        
                        # 如果找到不重叠位置或已尝试足够多次，就接受当前边界框
                    ...
                    for t in range(t_start, t_start + seq_length):
                        thumbnail = F.interpolate(
                            samples_view2[b, :, t, :, :].unsqueeze(0),
                            size=(thumbnail_size, thumbnail_size),
                            mode='bilinear',
                            align_corners=False
                        ).squeeze(0)
                        thumbnails.append(thumbnail)
                    all_thumbnails.append((torch.stack(thumbnails, dim=1), t_start))  # 存储缩略图和起始帧位置
                
                # 如果是双向嵌入，还需要为视角2准备视角1的缩略图
                view2_boxes = []
                all_view2_thumbnails = []
                
                if bidirectional_embedding:
                    for thumb_idx in range(num_thumbnails):
                        # 随机选择起始帧索引
                        t_start = random.randint(0, num_frames - seq_length)
                        ...
                        for attempt in range(max_attempts):
                            ...
                            # 检查与已有边界框是否重叠
                            overlap = False
                            for existing_box in view2_boxes:
                                if is_overlapping(box, existing_box, min_distance=2):
                                    overlap = True
                                    break
                        ...
                        for t in range(t_start, t_start + seq_length):
                            thumbnail = F.interpolate(
                                samples_view1[b, :, t, :, :].unsqueeze(0),
                                size=(thumbnail_size, thumbnail_size),
                                mode='bilinear',
                                align_corners=False
                            ).squeeze(0)
                            thumbnails.append(thumbnail)
                        all_view2_thumbnails.append((torch.stack(thumbnails, dim=1), t_start))
                
                # 将缩略图嵌入到视角1
                for thumb_idx in range(len(boxes)):
                    if thumb_idx >= num_thumbnails:
                        break
                        
                    bbx1, bby1, bbx2, bby2 = boxes[thumb_idx]
                    thumbnails, t_start = all_thumbnails[thumb_idx]
                    
                    # 将缩略图序列嵌入到对应帧中（使用透明度叠加）
                    for i in range(seq_length):
                        frame_idx = (t_start + i) % num_frames  # 循环嵌入，避免越界
                        
                        # 使用透明度叠加而不是完全替换
                        original_region = samples_view1[b, :, frame_idx, bbx1:bbx2, bby1:bby2].clone()
                        
                        # 透明度叠加
                        samples_view1[b, :, frame_idx, bbx1:bbx2, bby1:bby2] = original_alpha * original_region + overlay_alpha * thumbnails[:, i, :, :]
                
                # 如果是双向嵌入，还将视角1的缩略图嵌入到视角2
                if bidirectional_embedding:
                    for thumb_idx in range(len(view2_boxes)):
                        if thumb_idx >= num_thumbnails:
                            break
                            
                        bbx1, bby1, bbx2, bby2 = view2_boxes[thumb_idx]
                        thumbnails, t_start = all_view2_thumbnails[thumb_idx]
                        ...
                        for i in range(seq_length):
                            ...
                            # 使用透明度叠加而不是完全替换
                            original_region = samples_view2[b, :, frame_idx, bbx1:bbx2, bby1:bby2].clone()
                            
                            # 透明度叠加
                            samples_view2[b, :, frame_idx, bbx1:bbx2, bby1:bby2] = original_alpha * original_region + overlay_alpha * thumbnails[:, i, :, :]
