...
def train_class_batch(model, samples_view1, samples_view2, target, criterion, cur_epoch):
    ...
    print(f"0.5*lossnds1 epoch: {0.5 * lossnds1}")

    total_loss = 0.5 * lossnds1 + loss
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # 新增：定义缩略图大小和应用概率
    thumbnail_size = 75  # 可根据需要调整缩略图大小
...
# 方案二：
        ...
        #
        #     # 定义连续帧的长度（例如取 3 帧）
        #     seq_length = min(15, num_frames)  # 确保不超过总帧数
        ...
        #             frame_idx = (t_start + i) % num_frames  # 循环嵌入，避免越界
        #
        #             samples_view2[b, :, frame_idx, bbx1:bbx2, bby1:bby2] = thumbnails[:, i, :, :]

# 方案3
        # 缩略图嵌入逻辑 - 视角1嵌入视角2的前一帧和后一帧
        if random.random() < participation_rate:
            batch_size, _, num_frames, height, width = samples_view1.size()

            # 固定位置：左下角和右下角
            left_bottom_x1, left_bottom_y1 = 0, height - thumbnail_size  # [0, 149]
            left_bottom_x2, left_bottom_y2 = thumbnail_size, height  # [75, 224]
            right_bottom_x1, right_bottom_y1 = width - thumbnail_size, height - thumbnail_size  # [149, 149]
            right_bottom_x2, right_bottom_y2 = width, height  # [224, 224]

            # 对每个批次样本进行处理
            for b in range(batch_size):
                for frame_idx in range(num_frames):
                    # 前一帧 (t-1)
                    t_prev = max(0, frame_idx - 1)  # 边界处理
                    thumbnail_prev = F.interpolate(
                        samples_view2[b, :, t_prev, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0)  # [C, thumbnail_size, thumbnail_size]
                    ...
                    # 后一帧 (t+1)
                    t_next = min(num_frames - 1, frame_idx + 1)  # 边界处理
                    thumbnail_next = F.interpolate(
                        samples_view2[b, :, t_next, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0)  # [C, thumbnail_size, thumbnail_size]
                    ...
                    # 嵌入：左下角放前一帧，右下角放后一帧
                    samples_view1[b, :, frame_idx, left_bottom_x1:left_bottom_x2,
                    left_bottom_y1:left_bottom_y2] = thumbnail_prev
                    samples_view1[b, :, frame_idx, right_bottom_x1:right_bottom_x2,
                    right_bottom_y1:right_bottom_y2] = thumbnail_next

        # 缩略图嵌入逻辑 - 视角2嵌入视角1的前一帧和后一帧
        if random.random() < participation_rate:
            batch_size, _, num_frames, height, width = samples_view2.size()
            ...
            for b in range(batch_size):
                for frame_idx in range(num_frames):
                    ...
                    t_prev = max(0, frame_idx - 1)  # 边界处理
                    thumbnail_prev = F.interpolate(
                        samples_view1[b, :, t_prev, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0)  # [C, thumbnail_size, thumbnail_size]
                    ...
                    t_next = min(num_frames - 1, frame_idx + 1)  # 边界处理
                    thumbnail_next = F.interpolate(
                        samples_view1[b, :, t_next, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0)  # [C, thumbnail_size, thumbnail_size]
                    ...
                    # 嵌入：左下角放前一帧，右下角放后一帧
                    samples_view2[b, :, frame_idx, left_bottom_x1:left_bottom_x2,
                    left_bottom_y1:left_bottom_y2] = thumbnail_prev
                    samples_view2[b, :, frame_idx, right_bottom_x1:right_bottom_x2,
                    right_bottom_y1:right_bottom_y2] = thumbnail_next
