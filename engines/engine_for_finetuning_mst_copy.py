...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # 新增：定义缩略图大小和应用概率
    thumbnail_size = 45  # 可根据需要调整缩略图大小
    participation_rate = 0.8  # 80% 的概率应用缩略图嵌入
    ...
    for data_iter_step, (samples_view1, samples_view2, targets, _, _,_) in enumerate(
        ...
        # print(f'进入前samples_view1 shape: {samples_view1.shape}')
...
#         # 对视角二也操作
#         if random.random() < participation_rate:  # 随机决定是否应用缩略图增强
#             batch_size, _, num_frames, height, width = samples_view2.size()
#             # print("操作view2")
#
#             # 随机选择视角2的一个帧索引
#             t = random.randint(0, num_frames - 1)
...
#             for b in range(batch_size):
#                 # 提取视角1的第 t 帧并缩放为缩略图
#                 thumbnail_2 = F.interpolate(samples_view1[b, :, t, :, :].unsqueeze(0),
#                                           size=(thumbnail_size, thumbnail_size),
#                                           mode='bilinear',
#                                           align_corners=False).squeeze(0)
#
...
#                 # 将缩略图嵌入到视角1的每一帧的相同位置
#                 for frame in range(num_frames):
#                     samples_view2[b, :, frame, bbx1:bbx2, bby1:bby2] = thumbnail_2

# 方案二：
        # 缩略图嵌入逻辑 - 视角1嵌入视角2的动态连续帧
        if random.random() < participation_rate:
            batch_size, _, num_frames, height, width = samples_view1.size()

            # 定义连续帧的长度（例如取 3 帧）
            seq_length = min(3, num_frames)  # 确保不超过总帧数
            print("seq_length",seq_length)

            # 对每个批次样本进行处理
            for b in range(batch_size):
                # 随机选择起始帧索引
                t_start = random.randint(0, num_frames - seq_length)
                ...
                # 逐帧提取视角2的连续帧并缩放为缩略图
                thumbnails = []
                for t in range(t_start, t_start + seq_length):
                    thumbnail = F.interpolate(
                        samples_view2[b, :, t, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0)  # [C, thumbnail_size, thumbnail_size]
                    thumbnails.append(thumbnail)
                thumbnails = torch.stack(thumbnails, dim=1)  # [C, seq_length, thumbnail_size, thumbnail_size]

                # 第二个缩略图序列
                t_start2 = random.randint(0, num_frames - seq_length)  # 随机起始帧
                bbx1_2, bby1_2, bbx2_2, bby2_2 = rand_bbox(height, thumbnail_size)  # 没避免与第一个框重叠
                thumbnails2 = []
                for t in range(t_start2, t_start2 + seq_length):
                    thumbnail = F.interpolate(
                        samples_view2[b, :, t, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                    ...
                    thumbnails2.append(thumbnail)
                thumbnails2 = torch.stack(thumbnails2, dim=1)  # [C, seq_length, thumbnail_size, thumbnail_size]
                #
                # #第三个缩略图序列
                t_start3 = random.randint(0, num_frames - seq_length)  # 随机起始帧
                bbx1_3, bby1_3, bbx2_3, bby2_3 = rand_bbox(height, thumbnail_size)  # 没避免与第一个框重叠
                thumbnails3 = []
                for t in range(t_start3, t_start3 + seq_length):
                    thumbnail = F.interpolate(
                        samples_view2[b, :, t, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                    ...
                    thumbnails3.append(thumbnail)
                thumbnails3 = torch.stack(thumbnails3, dim=1)  # [C, seq_length, thumbnail_size, thumbnail_size]


        # # 缩略图嵌入逻辑 - 视角2嵌入视角1的动态连续帧
        # if random.random() < participation_rate:
        #     batch_size, _, num_frames, height, width = samples_view2.size()
        #     # 定义连续帧的长度
        #     seq_length = min(10, num_frames)
        #
        #     # 对每个批次样本进行处理
        #     for b in range(batch_size):
                # 逐帧提取视角1的连续帧并缩放为缩略图
                thumbnails_1 = []
                # 随机选择起始帧索引
                t_start_2 = random.randint(0, num_frames - seq_length)

                # 生成随机边界框（对所有帧使用相同位置）
                bbx1, bby1, bbx2, bby2 = rand_bbox(height, thumbnail_size)

                for t in range(t_start_2, t_start_2 + seq_length):
                    thumbnail = F.interpolate(
                        samples_view1[b, :, t, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                        align_corners=False
                    ).squeeze(0)  # [C, thumbnail_size, thumbnail_size]
                    thumbnails_1.append(thumbnail)
                thumbnails_1 = torch.stack(thumbnails_1, dim=1)  # [C, seq_length, thumbnail_size, thumbnail_size]

                # 第二个缩略图序列
                t_start2_2 = random.randint(0, num_frames - seq_length)  # 随机起始帧
                bbx1_2, bby1_2, bbx2_2, bby2_2 = rand_bbox(height, thumbnail_size)  # 未避免与第一个框重叠
                thumbnails2_2 = []
                for t in range(t_start2_2, t_start2_2 + seq_length):
                    thumbnail = F.interpolate(
                        samples_view1[b, :, t, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                    ...
                    thumbnails2_2.append(thumbnail)
                thumbnails2_2 = torch.stack(thumbnails2_2, dim=1)  # [C, seq_length, thumbnail_size, thumbnail_size]
                #
                # #第三个缩略图序列
                t_start3_2 = random.randint(0, num_frames - seq_length)  # 随机起始帧
                bbx1_3, bby1_3, bbx2_3, bby2_3 = rand_bbox(height, thumbnail_size)  # 没避免与第一个框重叠
                thumbnails3_2 = []
                for t in range(t_start3_2, t_start3_2 + seq_length):
                    thumbnail = F.interpolate(
                        samples_view1[b, :, t, :, :].unsqueeze(0),  # [1, C, H, W]
                        size=(thumbnail_size, thumbnail_size),
                        mode='bilinear',
                    ...
                    thumbnails3_2.append(thumbnail)
                thumbnails3_2 = torch.stack(thumbnails3_2, dim=1)  # [C, seq_length, thumbnail_size, thumbnail_size]

                # 将缩略图序列嵌入到视角1的对应帧中
                for i in range(seq_length):
                    frame_idx = (t_start + i) % num_frames  # 循环嵌入，避免越界
                    frame_idx2= (t_start2 + i) % num_frames
                    frame_idx3 = (t_start3 + i) % num_frames
                    samples_view1[b, :, frame_idx, bbx1:bbx2, bby1:bby2] = thumbnails[:, i, :, :]
                    samples_view1[b, :, frame_idx2, bbx1_2:bbx2_2, bby1_2:bby2_2] = thumbnails2[:, i, :, :]
                    samples_view1[b, :, frame_idx3, bbx1_3:bbx2_3, bby1_3:bby2_3] = thumbnails3[:, i, :, :]

                # 将缩略图序列嵌入到视角2的对应帧中
                for i in range(seq_length):
                    frame_idx = (t_start_2 + i) % num_frames  # 循环嵌入，避免越界
                    frame_idx2 = (t_start2_2 + i) % num_frames
                    frame_idx3 = (t_start3_2 + i) % num_frames
                    samples_view2[b, :, frame_idx, bbx1:bbx2, bby1:bby2] = thumbnails_1[:, i, :, :]
                    samples_view2[b, :, frame_idx2, bbx1_2:bbx2_2, bby1_2:bby2_2] = thumbnails2_2[:, i, :, :]
                    samples_view2[b, :, frame_idx3, bbx1_3:bbx2_3, bby1_3:bby2_3] = thumbnails3_2[:, i, :, :]

        # 在训练循环中保存所有视频
        # bsize = samples_view2.size(0)  # 获取批量大小
        # for b in range(bsize):
        #     output_path = f"/HOME/scw6fgn/run/VideoMamba-main/videomamba/video_sm/exp/k400/videomamba_small/video1/video2/epoch_{epoch}_batch_{data_iter_step}_sample2_{b}.mp4"  # 为每个样本生成唯一的文件名
        #     save_video_frames(samples_view2[b], output_path)
        #     output_path_2 = f"/HOME/scw6fgn/run/VideoMamba-main/videomamba/video_sm/exp/k400/videomamba_small/video1/video1/epoch_{epoch}_batch_{data_iter_step}_sample2_{b}.mp4"  # 为每个样本生成唯一的文件名
        #     save_video_frames(samples_view1[b], output_path_2)
...
def final_test(data_loader, model, device, file, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    # target_layer = base_model.layers[23].mixer.out_proj  # 选择最后一个 Mamba 块的输出投影层
    ...
    # plt.savefig('/HOME/scw6fgn/run/VideoMamba-main/videomamba/video_sm/exp/k400/videomamba_small/output/(3.31)_baseline+cut_random)/confusion_matrix_test.png')  # 保存到文件
    # plt.close()  # 关闭图表，避免显示
    # --- 可视化部分 ---
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    save_dir = os.path.dirname(file)
...
# final_test 是按样本（片段）级别计算准确率，每个片段独立预测。
# merge 是按视频级别计算准确率，先对所有片段特征平均，再预测。
...
def merge(eval_path, num_tasks):
    ...
    # 添加计数器
    total_segments = 0  # 总的视频片段数（包括重复）
    unique_segments = 0  # 非重复的视频片段数
    ...
    # 输出统计结果
    print(f"Total video segments processed (including duplicates): {total_segments}")
    print(f"Unique video segments processed: {unique_segments}")
    print(f"Number of unique videos: {len(dict_feats)}")
