...
def train_class_batch(model, samples_view1, samples_view2, target, criterion):
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples_view1, samples_view2, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if mixup_fn is not None:
            ...
            # print(f'Targets1 shape: {targets.shape}')
        ...
        if loss_scaler is None:
            ...
            loss, output = train_class_batch(model, samples_view1, samples_view2, targets, criterion)
        else:
            with amp_autocast:
                loss, output = train_class_batch(model, samples_view1, samples_view2, targets, criterion)
...
def validation_one_epoch(data_loader, model, device, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        if ds:
            ...
            # 将双视角数据输入模型
            output = model(videos_view1, videos_view2)
            loss = criterion(output, target)
        else:
            with amp_autocast:
                output = model(videos_view1, videos_view2)
        ...
        batch_size = videos_view1.shape[0]+videos_view2.shape[0]
