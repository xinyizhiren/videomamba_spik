...
def train_class_batch(model, samples, target, criterion):
    ...
    # print(f'samples_view2 shape: {samples_view2.shape}')

    # outputs = model(samples_view1, samples_view2)
    # 打印模型输出的形状，检查是否符合预期
    ...
    outputs, *rest = model(samples, samples)#,use_x2=False
    #print("***********************************************************************************************************************")  # 应为 (B, C)
    #print("outputs shape:", outputs.shape)  # 应为 (B, C)
    #print("target shape:", target.shape)    # 若用 SoftTargetCrossEntropy，需为 (B, C)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples_view1, samples_view2, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        samples_view2 = samples_view2.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)
        # print(f'Targets shape: {targets.shape}')
        original_targets = targets.clone()  # 使用 clone() 创建 targets 的副本
        # print(f'original_targets shape: {original_targets.shape}')
        original_targets = original_targets.to(device, non_blocking=True)

        if mixup_fn is not None:

            samples_view1, targets = mixup_fn(samples_view1, targets)
            samples_view2, original_targets = mixup_fn(samples_view2, original_targets)
            # print(f'samples_view1 shape: {samples_view1.shape}')
            # print(f'samples_view2 shape: {samples_view2.shape}')
            # print(f'mixTargets shape: {targets.shape}')
            # print(f'mixoriginal_targets shape: {original_targets.shape}')


            # print(f"Targets1 content: {targets}")
            # samples_view2, original_targets = mixup_fn(samples_view2, original_targets)
            # print(f'Targets2 shape: {targets.shape}')
            # print(f"Targets2 content: {targets}")
        ...
        if loss_scaler is None:
            if not no_amp:
                ...
                # mix_samples = mix_samples.bfloat16() if bf16 else mix_samples.half()
            loss_1, output_1 = train_class_batch(model,samples_view1, targets,criterion)
            loss_2, output_2 = train_class_batch(model, samples_view2, original_targets, criterion)
            loss=loss_1+loss_2
        else:
            with amp_autocast:
                loss_1, output_1 = train_class_batch(model, samples_view1, targets, criterion)
        ...
        if mixup_fn is None:
            ...
            print("mix=none,output shape: {}".format(output.shape))
