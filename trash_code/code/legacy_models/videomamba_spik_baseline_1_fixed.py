...
__all__ = ['SpikMambaFixed', 'spikmamba_fixed']
...
class SSM_Block(nn.Module):
    ...
    def forward(
        ...
        if use_checkpoint:
            mixed = checkpoint.checkpoint(self.mixer, x, inference_params)
        else:
            mixed = self.mixer(x, inference_params=inference_params)
...
class TemporalBranch(nn.Module):
    def __init__(self, img_size=128, patch_size=8, in_chans=3, embed_dims=256, layer_nums=[8, 4, 2],
                 ...
                 rms_norm=True, residual_in_fp32=True, fused_add_norm=False, bimamba=True, kernel_size=1, drop_path_rate=0.1):
        super().__init__()
        ...
        if num_frames < kernel_size:
            raise ValueError("num_frames must be greater than or equal to tubelet_size/kernel_size.")
        self.patch_frames = (num_frames - kernel_size) // kernel_size + 1
        ssm_dpr = [x.item() for x in torch.linspace(0, drop_path_rate, 12)]
        ssm_inter_dpr = [0.0] + ssm_dpr
        factory_kwargs = {"device": None, "dtype": None}

        self.spatial_size = img_size//patch_size

        self.patch_embed1 = PatchEmbeding(in_channels=embed_dims, out_channels=embed_dims, expansion=expansion, mode='init')
        # 构建Mamba层
        self.mamba_layers1 = nn.ModuleList(
            [
                create_block(
                    d_model=ssm_embed_dims,ssm_cfg=None,layer_idx=i, time_steps=T*self.patch_frames,
                    drop_path=ssm_inter_dpr[i],mlp_ratio=mlp_ratio,bimamba=bimamba,**factory_kwargs,)
                for i in range(layer_nums[0])  # 保持1层Mamba
            ]
        )
        
        #[T, B, C, H, W]
        self.patch_embed2 = PatchEmbeding(in_channels=embed_dims, out_channels=embed_dims, expansion=embed_ratio, mode='stage')
        ...
        self.patch_embed3 = PatchEmbeding(in_channels=embed_dims*2, out_channels=embed_dims, expansion=embed_ratio, mode='stage')
    ...
    def forward(self, x, inference_params=None):
        ...
        BT ,N , C = x.shape
        frame_count = BT // batch_size
        if frame_count != self.patch_frames:
            raise ValueError(
                f"SpikMambaFixed expected {self.patch_frames} embedded frames from "
                f"num_frames={self.num_frames}, tubelet_size={self.tubelet_size}, got {frame_count}. "
                "Pass matching num_frames/tubelet_size when creating the model.")
        x = x.view(batch_size, frame_count, N, C).flatten(1, 2) # [B, T*N, C]
        # 时序位置嵌入
        temporal_pos_emb = self._get_temporal_pos_emb(batch_size, frame_count, N)
        ...
        x.shape
        x_reversed.shape
        x_concat = torch.cat([x, x_reversed], dim=2)
        x_concat.shape
    ...
    def _get_temporal_pos_emb(self, B, num_frames, spatial_tokens):
        # 生成时序位置嵌入 [B, N, C]
        temporal_pos = torch.arange(num_frames, device=self.norm_f.weight.device)
        temporal_emb = nn.functional.one_hot(temporal_pos, num_classes=self.ssm_embed_dims).float()
        temporal_emb = temporal_emb.unsqueeze(1).repeat(1, spatial_tokens, 1)  # 每个时间步对应相同空间位置
...
class SpikMambaFixed(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=8, in_channels=3, 
                 num_classes=300, rms_norm=True, residual_in_fp32=True, embed_dims=256,mlp_ratios=4.0,depths=2,
                 fused_add_norm=False, bimamba=True,sr_ratios=1,
                 num_frames=16, batch_size=8, sigma_x=1.0, sigma_y=None, T=4, kernel_size=1, drop_path_rate=0.1):
        super().__init__()
    ...
    def _init_hsic_sigma(self, feat):
        ...
        with torch.no_grad():
            self.hsic_criterion.sigma_x = self.hsic_criterion.sigma_y = torch.std(feat).item() * 2  # 自适应设置带宽


    def median_heuristic_sigma_estimator(self, feat: torch.Tensor) -> float:
        """
        使用中位数启发式（Median Heuristic）估算 RBF 核函数的带宽参数 sigma。
        sigma 被设置为 Batch 中所有样本对之间 L2 距离的中位数。
        这是 HSIC/MMD 损失中更鲁棒、更标准的 sigma 估算方法。

        Args:
            feat (torch.Tensor): 输入特征张量，形状通常为 [Batch_Size, Feature_Dim]。
        ...
        # 确保在 no_grad 环境下进行计算
        with torch.no_grad():
            X_sqnorms = torch.sum(feat ** 2, dim=1)  # Shape: [N]
            XXT = feat @ feat.t()  # Shape: [N, N]
            X_L2 = -2 * XXT + X_sqnorms.unsqueeze(1) + X_sqnorms.unsqueeze(0)
            X_L2 = torch.clamp(X_L2, min=1e-12)
            N = X_L2.shape[0]
            if N < 2:
                return 1.0 
            upper_triangular_indices = torch.triu_indices(N, N, offset=1)
            pairwise_distances = torch.sqrt(X_L2[upper_triangular_indices[0], upper_triangular_indices[1]])
            median_distance = torch.median(pairwise_distances)
            
            self.hsic_criterion.sigma_x = self.hsic_criterion.sigma_y = median_distance.item()


    def forward_features(self, x):
        """提取特征并生成分类logits（原始证据）"""
        feat = self.temporal_branch(x) # [B, T, C]
        logits = self.classifier(self.head_drop(feat.mean(dim=1)))  # [B, num_classes]（原始证据）
        return logits, feat  # 返回logits和特征（用于融合和HSIC）

    def forward(self, x1, x2=None, inference_params=None, hsic=True, return_view_logits=False):
        # 单视角处理
        ...
        if return_view_logits:
            return fused_logits, logits1, logits2, alpha1, alpha2

        # 3. HSIC损失（训练阶段约束视角一致性）
        B, T, C = feat1.shape
        feat1 = feat1.view(B*T,C)
        feat2 = feat2.view(B*T,C)
        
        if hsic:
            # 自动初始化HSIC的sigma（首次调用时）
            if self.auto_sigma and self.hsic_criterion.sigma_x == 1.0:
                self.hsic_criterion.sigma_x = self.hsic_criterion.sigma_y = 0.1
...
# 注册模型
@register_model
def dual_stream_spikmamba_fixed(pretrained=False, pretrained_cfg=None, **kwargs):
...
# 注册模型
@register_model
def spikmamba_fixed(pretrained=False, pretrained_cfg=None, **kwargs):
    kwargs.pop('pretrained_cfg_overlay', None)
    drop_path_rate = kwargs.pop('drop_path_rate', kwargs.pop('drop_path', 0.1))
    kernel_size = kwargs.pop('kernel_size', None)
    kwargs.pop('use_checkpoint', None)
    kwargs.pop('checkpoint_num', None)
    all_frames = kwargs.pop('all_frames', None)
    tubelet_size = kwargs.pop('tubelet_size', None)
    if kernel_size is None:
        kernel_size = tubelet_size if tubelet_size is not None else 1
    kwargs.pop('use_learnable_pos_emb', None)
    kwargs.pop('fc_drop_rate', None)
    kwargs.pop('drop_rate', None)
    kwargs.pop('attn_drop_rate', None)
    kwargs.pop('drop_block_rate', None)
    kwargs.pop('use_mean_pooling', None)
    kwargs.pop('init_scale', None)

    img_size_h = kwargs.pop('img_size_h', kwargs.pop('img_size', 224))
    img_size_w = kwargs.pop('img_size_w', img_size_h)
    patch_size = kwargs.pop('patch_size', 14)
    embed_dims = kwargs.pop('embed_dims', 384)
    time_steps = kwargs.pop('T', kwargs.pop('time_steps', 4))
    model = SpikMambaFixed(
        img_size_h=img_size_h, img_size_w=img_size_w, patch_size=patch_size,
        embed_dims=embed_dims, T=time_steps, kernel_size=kernel_size,
        mlp_ratios=kwargs.pop('mlp_ratios', 4.0),
        in_channels=kwargs.pop('in_channels', kwargs.pop('in_chans', 3)),
        batch_size=kwargs.pop('batch_size', 2),
        num_frames=kwargs.pop('num_frames', all_frames if all_frames is not None else 8),
        depths=kwargs.pop('depths', 2),
        sr_ratios=kwargs.pop('sr_ratios', 1),
        num_classes=kwargs.pop('num_classes', 12),
        bimamba=kwargs.pop('bimamba', True),
        rms_norm=kwargs.pop('rms_norm', True),
        residual_in_fp32=kwargs.pop('residual_in_fp32', True),
        fused_add_norm=kwargs.pop('fused_add_norm', False),
        drop_path_rate=drop_path_rate,
    )
...
if __name__ == '__main__':
    ...
    model = SpikMambaFixed(
        img_size_h=224, img_size_w=224, patch_size=14, embed_dims=384, T = 4,
        mlp_ratios=4.0, in_channels=3, num_classes=60,batch_size=1,
        num_frames=8, depths=2, sr_ratios=1
    ).cuda()

    input_tensor1 = torch.randn(1, 3, 8, 224, 224).cuda()  # [B, C, T, H, W]
    input_tensor2 = torch.randn(1, 3, 8, 224, 224).cuda()  # [B, C, T, H, W]
    output = model(input_tensor1, input_tensor2)[0]
