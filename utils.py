...
def multiple_samples_collate(batch, fold=False):
    ...
    """
    # 解包 batch 中的所有数据
    frame_list_view1, frame_list_view2, label_list, index_list,index_list2, extra_data = zip(*batch)

    # 展开嵌套的列表（视角1、视角2的图像）
    frame_list_view1 = [item for sublist in frame_list_view1 for item in sublist]
    frame_list_view2 = [item for sublist in frame_list_view2 for item in sublist]
    label_list = [item for sublist in label_list for item in sublist]
    index_list = [item for sublist in index_list for item in sublist]
    index_list2 = [item for sublist in index_list2 for item in sublist]
    # 使用 default_collate 合并数据
    frame_list_view1 = default_collate(frame_list_view1)
    frame_list_view2 = default_collate(frame_list_view2)
    label_list = default_collate(label_list)
    index_list = default_collate(index_list)
    index_list2 = default_collate(index_list2)
    extra_data = default_collate(extra_data)

    # 如果 fold 为 True，返回四个元素，注意这里只返回一次视角数据
    if fold:
        return [frame_list_view1], label_list, index_list, index_list2,extra_data
    else:
        # 否则，返回两个视角的图像数据，标签和索引
        return frame_list_view1, frame_list_view2, label_list, index_list, index_list2,extra_data
