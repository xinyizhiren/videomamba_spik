...
class Block(nn.Module):
    def __init__(
            self, dim, mixer_cls, norm_cls=nn.LayerNorm, fused_add_norm=False, residual_in_fp32=False, drop_path=0., timesteps=8
    ):
        """
        ...
        self.norm = norm_cls(dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')  # 添加 LIF 神经元
        self.timesteps = timesteps  # 新增：时间步数
        if self.fused_add_norm:
            assert RMSNorm is not None, "RMSNorm import fails"
            assert isinstance(
                self.norm, (nn.LayerNorm, RMSNorm)
    ...
    def forward(
        ...
        # 重置 LIF 膜电位为零张量
        # self.lif.v = torch.zeros_like(hidden_states[0])  # 匹配时间步 t=0 的形状
        ...
        if use_checkpoint:
            ...
            hidden_states = self.mixer(hidden_states, inference_params=inference_params)
        # print(f"[SpikingBlock] lif input shape: {hidden_states.shape}")
        # print(f"[SpikingBlock] lif input mean: {hidden_states.mean().item():.4f}, std: {hidden_states.std().item():.4f}")
        hidden_states = hidden_states.unsqueeze(0).repeat(self.timesteps, 1, 1, 1)
        hidden_states = self.lif(hidden_states)  # 应用 LIF 神经元
        hidden_states = hidden_states.sum(dim=0)  # 时间维度求和，[B, N, D]

        # 调试信息：LIF 输出（脉冲）
        # spike_rate = (hidden_states == 1).float().mean().item()
        # print(f"[SpikingBlock] lif output shape: {hidden_states.shape}")
        # print(f"[SpikingBlock] lif output spike rate: {spike_rate:.4f}, unique values: {torch.unique(hidden_states.float()).tolist()}")
        return hidden_states, residual
...
class SpikingPatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, kernel_size=1, in_chans=3, embed_dim=768, timesteps=8):
        super().__init__()
        ...
        self.proj_bn = nn.BatchNorm3d(embed_dim // 4)
        ...
        self.proj2_bn = nn.BatchNorm3d(embed_dim)
        self.proj2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        # 初始化批归一化层
        nn.init.constant_(self.proj_bn.weight, 1.0)
        nn.init.constant_(self.proj_bn.bias, 0.0)
        nn.init.constant_(self.proj2_bn.weight, 1.0)
        nn.init.constant_(self.proj2_bn.bias, 0.0)

    def forward(self, x):
        # 重置 LIF 膜电位为零张量
        # self.proj_lif.v = torch.zeros_like(self.proj(x)[0])  # 匹配时间步 t=0 的形状
        # self.proj2_lif.v = torch.zeros_like(self.proj2(x)[0])
        B, C, T, H, W = x.shape
        print(f"Input shape: {x.shape}")
        # print("embed x.shape：",x.shape)
        x = self.proj(x)  # [B, embed_dim//4, T', H', W']
        x = self.proj_bn(x)
        # 引入 timesteps 维度，为 proj_lif 准备
        print(f"After proj shape: {x.shape}")
        x = x.unsqueeze(0).repeat(self.timesteps, 1, 1, 1, 1, 1)  # [timesteps, B, embed_dim//4, T', H', W']
        x = x.view(self.timesteps, B, -1, T, H // 16, W // 16)  # 确保形状正确
        print(f"Before proj_lif shape: {x.shape}")

        # 调试信息：第一阶段 LIF 输入
        # print(f"[SpikingPatchEmbed] proj_lif input shape: {x.shape}")
        # print(f"[SpikingPatchEmbed] proj_lif input mean: {x.mean().item():.4f}, std: {x.std().item():.4f}")
        x = self.proj_lif(x)
        x = x.sum(dim=0)  # 时间维度求和，得到 [B, embed_dim//4, T', H', W']

        # 调试信息：第一阶段 LIF 输出（脉冲）
        # spike_rate = (x == 1).float().mean().item()  # 计算脉冲发放率
        # print(f"[SpikingPatchEmbed] proj_lif output shape: {x.shape}")
        # print(f"[SpikingPatchEmbed] proj_lif output spike rate: {spike_rate:.4f}, unique values: {torch.unique(x.float()).tolist()}")
        x = self.proj2(x)  # [B, embed_dim, T', H', W']
        x = self.proj2_bn(x)
        print(f"After proj2 shape: {x.shape}")
        # 调试信息：第二阶段 LIF 输入
        # print(f"[SpikingPatchEmbed] proj2_lif input shape: {x.shape}")
        # print(f"[SpikingPatchEmbed] proj2_lif input mean: {x.mean().item():.4f}, std: {x.std().item():.4f}")
        # 引入 timesteps 维度，为 proj2_lif 准备
        x = x.unsqueeze(0).repeat(self.timesteps, 1, 1, 1, 1, 1)  # [timesteps, B, embed_dim, T', H', W']
        x = x.view(self.timesteps, B, -1, T, H // 16, W // 16)
        print(f"Before proj2_lif shape: {x.shape}")

        x = self.proj2_lif(x)
        print(f"After proj2_lif shape: {x.shape}, spike rate: {(x == 1).float().mean().item():.4f}")
        x = x.sum(dim=0)  # 时间维度求和，得到 [B, embed_dim, T', H', W']
        print(f"After proj2_lif integration shape: {x.shape}")
        # 调试信息：第二阶段 LIF 输出（脉冲）
        # spike_rate = (x == 1).float().mean().item()
        # print(f"[SpikingPatchEmbed] proj2_lif output shape: {x.shape}")
        # print(f"[SpikingPatchEmbed] proj2_lif output spike rate: {spike_rate:.4f}, unique values: {torch.unique(x.float()).tolist()}")
        return x  # 返回 [B, embed_dim, T', H', W']
...
class VisionMamba(nn.Module):
        ...
        for i in range(depth):
            if i < 2:  # 前 2 层使用含 LIF 的 Block
                block = create_block(
                    ...
                    device=device,
                    dtype=dtype,
                    timesteps=timesteps,  # 新增参数
                )
            else:  # 后 22 层使用不含 LIF 的 PlainBlock
                block = create_plain_block(
                    embed_dim,
                    ssm_cfg=ssm_cfg,
                    norm_epsilon=norm_epsilon,
                    rms_norm=rms_norm,
    ...
    # def _reset_all_lif_neurons(self):
    #     """重置模型中所有的LIF神经元状态"""
    #     # 重置SpikingPatchEmbed中的LIF神经元
    #     if hasattr(self.patch_embed, 'proj_lif'):
    #         self.patch_embed.proj_lif.reset()
    #     if hasattr(self.patch_embed, 'proj2_lif'):
    #         self.patch_embed.proj2_lif.reset()
    #
    #     # 重置所有SpikingBlock中的LIF神经元
    #     for layer in self.layers:
    #         if hasattr(layer, 'lif'):
    #             layer.lif.reset()
    #         # 支持Block类
    #         elif hasattr(layer, 'mixer') and hasattr(layer.mixer, 'lif'):
    #             layer.mixer.lif.reset()
    def _reset_all_lif_neurons(self):
        """重置模型中所有的 LIF 神经元状态"""
        # 重置 SpikingPatchEmbed 中的 LIF 神经元
        if hasattr(self.patch_embed, 'proj_lif'):
            self.patch_embed.proj_lif.reset()
        if hasattr(self.patch_embed, 'proj2_lif'):
            self.patch_embed.proj2_lif.reset()

        # 只重置前 4 层 SpikingBlock 中的 LIF 神经元
        for i, layer in enumerate(self.layers[:2]):  # 仅遍历前 4 层
            if hasattr(layer, 'lif'):
                layer.lif.reset()
...
#     return weight_3d
def inflate_weight(weight_2d, time_dim, center=True):
    print(f'Init center: {center}, Input shape: {weight_2d.shape}, time_dim: {time_dim}')
    if weight_2d.dim() == 5:
        # 如果已经是 5D，直接返回（无需膨胀）
        return weight_2d
    elif weight_2d.dim() == 4:
        # 4D 权重，插入时间维度
        weight_3d = weight_2d.unsqueeze(2)
        if time_dim > 1:
            weight_3d = weight_3d.repeat(1, 1, time_dim, 1, 1)
            if center:
                weight_3d = torch.zeros_like(weight_3d)
                middle_idx = time_dim // 2
                weight_3d[:, :, middle_idx, :, :] = weight_2d
            else:
                weight_3d = weight_3d / time_dim
    else:
        raise ValueError(f"Unsupported weight dimension: {weight_2d.dim()}")
    print(f'Output shape: {weight_3d.shape}')
    return weight_3d
    ...
    for k in state_dict.keys():
        if k in state_dict_3d.keys():
            if k == "patch_embed.proj.weight":
                # 截取前 embed_dim//4 个通道
                state_dict[k] = state_dict[k][:state_dict_3d[k].shape[0], :, :, :, :]
                print(f"Load: {k}, {state_dict[k].shape} => {state_dict_3d[k].shape}")
            elif k == "patch_embed.proj.bias":
                # 截取前 embed_dim//4 个 bias
                state_dict[k] = state_dict[k][:state_dict_3d[k].shape[0]]
                print(f"Load: {k}, {state_dict[k].shape} => {state_dict_3d[k].shape}")
            elif k == "patch_embed.proj2.weight":
                # proj2 没有预训练权重，随机初始化或从原始权重提取
                proj2_weight = torch.zeros_like(state_dict_3d[k])
                for i in range(state_dict_3d[k].shape[0]):  # 384
                    for j in range(state_dict_3d[k].shape[1]):  # 96
                        # 从原始权重的中心区域提取
                        proj2_weight[i, j, 0, :, :] = state_dict["patch_embed.proj.weight"][i, j % 3, 0, 7:10, 7:10]
                state_dict[k] = proj2_weight
                print(f"Init: {k}, shape: {state_dict[k].shape}")
            elif k == "patch_embed.proj2.bias":
                # 复用原始 bias 或初始化为零
                state_dict[k] = state_dict["patch_embed.proj.bias"]
                print(f"Load: {k}, {state_dict[k].shape} => {state_dict_3d[k].shape}")
            elif state_dict[k].shape != state_dict_3d[k].shape and len(state_dict_3d[k].shape) > 3:
                # 其他需要膨胀的权重
                print(f'Inflate: {k}, {state_dict[k].shape} => {state_dict_3d[k].shape}')
                time_dim = state_dict_3d[k].shape[2]
                state_dict[k] = inflate_weight(state_dict[k], time_dim, center=center)
            else:
                print(f"Direct load: {k}, {state_dict[k].shape}")
        else:
            print(f'Ignore: {k}')

    # 删除 head 权重（因为 num_classes 可能不同）
