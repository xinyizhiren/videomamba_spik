...
class SpikingPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # 引入 timesteps 维度，为 LIF 准备
        x = x.unsqueeze(0).repeat(self.timesteps, 1, 1, 1, 1, 1)  # [timesteps, B, embed_dim, T', H', W']
        x = x.view(self.timesteps, B, self.embed_dim, T, H // self.patch_size[0], W // self.patch_size[1])
        print(f"Before lif shape: {x.shape}")

        x = self.lif(x)  # 应用 LIF 神经元，输出脉冲
        print(f"After lif shape: {x.shape}, spike rate: {(x == 1).float().mean().item():.4f}")
        x = x.sum(dim=0)  # 时间维度求和，恢复 [B, embed_dim, T', H', W']
        print(f"After lif integration shape: {x.shape}")
...
class VisionMamba(nn.Module):
    ...
    def _reset_all_lif_neurons(self):
        if hasattr(self.patch_embed, 'lif'):
            self.patch_embed.lif.reset()
