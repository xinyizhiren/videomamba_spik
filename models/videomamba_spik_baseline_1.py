...
class TemporalBranch(nn.Module):
    def __init__(self, img_size=128, patch_size=8, in_chans=3, embed_dims=256, layer_nums=[8, 4, 2],
        ...
        self.mamba_layers1 = nn.ModuleList(
            [
                create_block(
                    d_model=ssm_embed_dims,ssm_cfg=None,layer_idx=i, time_steps=T*num_frames,
...
class SpikMamba(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=8, in_channels=3, 
        ...
        self.classifier = nn.Linear(embed_dims*num_frames*T, num_classes)  # 分类头（生成原始证据）
    ...
    def forward_features(self, x):
        ...
        B, T, C = feat.shape
        logits = self.classifier(feat.view(B, T*C))  # [B, num_classes]（原始证据）
        return logits, feat  # 返回logits和特征（用于融合和HSIC）

    def forward(self, x1, x2=None, inference_params=None, hsic=True):
        # 单视角处理
...
# 注册模型
@register_model
def spikmamba(pretrained=False,pretrained_cfg=None, batch_size=2, num_classes=12):
    model = SpikMamba(
        img_size_h=224, img_size_w=224, patch_size=14, embed_dims=384,T = 4,
        mlp_ratios=4.0, in_channels=3,batch_size=batch_size,
        num_frames=8, depths=2, sr_ratios=1, num_classes=num_classes,
    )
...
if __name__ == '__main__':
    ...
    model = SpikMamba(
        img_size_h=224, img_size_w=224, patch_size=14, embed_dims=384, T = 4,
        num_heads=8, mlp_ratios=4.0, in_channels=3, num_classes=60,batch_size=1,
