...
import json
import os
import torch.distributed as dist  # 确保导入了 distributed 模块
...
def main(args, ds_init):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.distributed:
            ...
            print(f"Epoch {epoch} - Sampler setup time: {sampler_end - sampler_start:.4f}s")
        ...
        print(f"Epoch {epoch} - Training time: {train_duration:.4f}s")
    ...
    # ✅ 安全 barrier：仅在分布式模式下调用
    if dist.is_available() and dist.is_initialized():
        dist.barrier()
    else:
        print("⚠️ [Info] Running in non-distributed mode, skipping dist.barrier()")

    # ✅ 仅主进程执行结果合并与日志记录
    if (not dist.is_available() or not dist.is_initialized()) or dist.get_rank() == 0:
        print("Start merging results...")

        # 在单卡模式下，num_tasks 设置为 1
        ...
        print(f"Accuracy of the network on the {len(dataset_test)} test videos: "
            f"Top-1: {final_top1:.2f}%, Top-5: {final_top5:.2f}%")
        ...
        if args.output_dir and (not dist.is_available() or not dist.is_initialized() or utils.is_main_process()):
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                f.write(json.dumps(log_stats) + "\n")
